...
그룹 함수는 테이블에서 데이터를 뽑아 분류통에 나눠서 담는다. (group by)

각 분류통 안에서만 따로따로 집계(합, 평균 등등...)를 수행

mysql 기준
테이블 스캔 -> 그룹핑 -> 그룹별로 집계 -> 반환

만약에 기술면접에서
그룹 함수의 작동원리가 뭐야? 라고 물어보면
